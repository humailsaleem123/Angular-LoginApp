import { Component, ViewChild } from '@angular/core';
import { IgxTooltipDirective } from './tooltip.directive';
import * as i0 from "@angular/core";
import * as i1 from "./tooltip.directive";
export class IgxTooltipComponent {
}
IgxTooltipComponent.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "14.0.0", ngImport: i0, type: IgxTooltipComponent, deps: [], target: i0.ɵɵFactoryTarget.Component });
IgxTooltipComponent.ɵcmp = i0.ɵɵngDeclareComponent({ minVersion: "14.0.0", version: "14.0.0", type: IgxTooltipComponent, selector: "igx-tooltip", viewQueries: [{ propertyName: "tooltip", first: true, predicate: IgxTooltipDirective, descendants: true, static: true }], ngImport: i0, template: "<span igxTooltip>{{content}}</span>", dependencies: [{ kind: "directive", type: i1.IgxTooltipDirective, selector: "[igxTooltip]", inputs: ["context", "id"], exportAs: ["tooltip"] }] });
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "14.0.0", ngImport: i0, type: IgxTooltipComponent, decorators: [{
            type: Component,
            args: [{ selector: 'igx-tooltip', template: "<span igxTooltip>{{content}}</span>" }]
        }], propDecorators: { tooltip: [{
                type: ViewChild,
                args: [IgxTooltipDirective, { static: true }]
            }] } });
//# sourceMappingURL=data:application/json;base64,